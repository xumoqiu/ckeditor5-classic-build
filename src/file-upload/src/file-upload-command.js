/* global document, FormData, setTimeout */
...
export default class InsertSjkFileUploadEditorCommand extends Command {
	execute() {
		...
		uploadInput.onchange = e => {
			changeUploadInput.call( uploadInput, e, this, this.editor );
		};
		uploadInput.click();
	}
 ...
 *  创建一个上传zip的dom
 * **/
function changeUploadInput( e, commandInstance, editor ) {
	const form = new FormData();
	const name = this.files[ 0 ].name;
	const fileName = this.files[ 0 ].name;
	const lastDotIndex = fileName.lastIndexOf( '.' );
	const ext = fileName.substring( lastDotIndex + 1, fileName.length ).toLocaleLowerCase();
	const size = this.files[ 0 ].size / 1024 / 1024;

	const baseUrl = commandInstance.editor.config._config.uploadFile.baseUrl;
	const uploadUrl = commandInstance.editor.config._config.uploadFile.uploadUrl;
	const ckEditorId = commandInstance.editor.config._config.uploadFile.ckEditorId;
	...
	// form.append(commandInstance.editor.config._config.uploadFile.key1, this.files[0]);
	// form.append(commandInstance.editor.config._config.uploadFile.key2, 'ckeditorFile');

	form.append( 'file', this.files[ 0 ] );
	form.append( 'UploadFolder', commandInstance.editor.config._config.uploadFile.key );
